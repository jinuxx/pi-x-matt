...
本 skill 是手动调用的 architecture survey，不是 refactor runner。一次运行只生成 OS temp HTML report，并最多深入一个用户选择的 candidate；不修改生产代码、不写 tests、不创建 implementation tickets、不进入 `matt-implement`。选定并确认的 deepening decision 后续重新进入 `to-spec → to-tickets → implement`。

开始时完整读取并应用 [matt-grilling](../matt-grilling/SKILL.md) 与 [matt-domain-modeling](../matt-domain-modeling/SKILL.md)。父会话拥有 scope、candidate choice、HITL 和领域文档写入；`architecture-scan` 的唯一 worker 只写 temp report；`architecture-design` 的三个 reader 只返回 interface proposals。
...
围绕选定 candidate 应用 `matt-grilling` decision tree，依次确认：
...
- 决定仍需进入 `matt-to-spec`，再由 `matt-to-tickets` 切 tracer-bullet slices，并由 `matt-implement` 执行 TDD/review；
