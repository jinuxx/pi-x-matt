...
metadata:
  ...
  pi-dispatch: none
  pi-depends-on: matt-grilling, matt-domain-modeling, matt-to-spec
...
用 `matt-grilling` 的 decision tree 与 `matt-domain-modeling` 的术语/ADR 纪律，先确定 destination、范围、非目标和完成地图的可观察条件。使用 `ask_user_question` 取得明确确认；确认前不得创建 map 或 ticket。
...
若没有 fog、所有路线已清楚且整体可在一个 session 处理，停止并让用户选择 `matt-grill-with-docs`、`matt-to-spec` 或一个小型 `matt-implement` slice；不要为了使用 Wayfinder 而创建空 map。
...
清空后重新读取 `matt-to-spec`，把 **cleared map reference**、`Decisions so far` pointers、相关 ticket answers、领域文档和 ADR 交给它综合。不要把 decision tickets 当成 implementation tickets，不要直接进入 `matt-to-tickets` 或 `matt-implement`，也不要在 Wayfinder 中生成 spec。报告地图已清空并停止，等待用户明确进入下一阶段。
