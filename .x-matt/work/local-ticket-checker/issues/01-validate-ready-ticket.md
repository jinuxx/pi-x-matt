...
提供 `node scripts/check-local-ticket.mjs <repo-relative-ticket-path>`，验证仓库内 `.x-matt/work/` ticket 的入口 metadata，并通过退出码和 JSON 返回结果。本 slice 只处理 `Blocked by: None`。

## Acceptance criteria

- [x] 输入 `Type: ticket`、`Status: ready-for-agent`、`Blocked by: None` 的有效文件时 exit 0，JSON `ok` 为 true。
- [x] 输入 spec、错误状态、缺失必需字段、绝对路径、仓库外路径或 `.x-matt/work/` 外路径时 exit 1。
