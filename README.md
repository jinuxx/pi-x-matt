...
因此项目不会加载包内 `pi-subagents`/`council-mode` skills、prompt templates 或 builtin agents。项目只暴露四个自定义 agent；项目级 reviewer 固定使用 `deepseek/deepseek-v4-flash`，worker 保持当前父会话模型：
